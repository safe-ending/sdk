 ...
 */
public class SpinMenuLayout extends ViewGroup implements Runnable, View.OnClickListener, View.OnLongClickListener {

    public static final String TAG = SpinMenuLayout.class.getSimpleName();
    /**
     * View 之间间隔的角度
     */
    private static final int ANGLE_SPACE = 25;//45
     ...
     * 用于自动滚动时速度加快，无其他意义
     */
    private static final float ACCELERATE_ANGLE_RATIO = 1.0f;//1.8

    /**
     * 用于加长半径，无其他意义
     */
    private static final float RADIUS_HALF_WIDTH_RATIO = 1.8f;//1.2
    private static final float RADIUS_HALF_HEIGHT_RATIO = 1.0f;//1.2
    ...
    private OnMenuLongClickListener onMenuLongClickListener;
    ...
    protected void onLayout(boolean b, int left, int top, int right, int bottom) {
        ...
        int centerX = getMeasuredWidth() / 2;//旋转的圆心点x
        //        int centerY = getMeasuredHeight();//旋转的圆心点y
        int centerY = getMeasuredHeight() + getChildAt(0).getMeasuredHeight() / 2;//旋转的圆心点y
        //        radius = centerX * RADIUS_HALF_WIDTH_RATIO + getChildAt(1).getMeasuredHeight() / 2;
        radius = getChildAt(0).getMeasuredHeight();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //            Log.e(TAG, i+" 半径：radius="+radius+", left="+left+", right="+right+", top="+top+", bottom="+bottom+
            //                    ", 圆心：centerX="+centerX+", centerY="+centerY+", 子视图宽高：childWidth="+childWidth+", childHeight="+childHeight);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(perAngle) > 2 || Math.abs(perDiffAngle) > 2) {
                    isLongClick = false;
                }
                if (isLongClick && Math.abs(perAngle) < 2 && (System.currentTimeMillis() - preTimes) > 800) {
            ...
            case MotionEvent.ACTION_UP:
                // 判断是否长按
                long interval = System.currentTimeMillis() - preTimes;
                if (isLongClick && interval > 500) {
                    onLongClick();
                    return true;
                }
                // 不是长按
                anglePerSecond = perAngle * 1000 / interval;
    ...
    public void onClick(View view) {
        ...
        Log.i(TAG, "cur index=" + index + ",last selPos=" + selPos);
        ...
        if (Math.abs(perAngle) <= touchSlopAngle) {
            if (index != selPos) {
                ...
                if (view instanceof SMItemLayout
                        ...
                        && enable) {
                    onMenuSelectedListener.onMenuSelected((SMItemLayout) view, selPos);
                }
    ...
    public void scroll(int startX, int startY, int clickIndex, int currSelPos) {
        scroller.startScroll(startX, startY, computeClickToEndAngle(clickIndex, currSelPos), 0, 300);
        post(this);
    }
    ...
    public boolean onLongClick() {
        int selPos = getSelectedPosition();
        //        Log.e(TAG, "----------------------------------------");
        Log.e(TAG, "long click selPos=" + selPos);
        onMenuLongClickListener.onMenuLongClick(selPos);
        return false;
    }
