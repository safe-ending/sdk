...
//            PdfUtil.pdfModel(mStrokeView, mStrokeView.getWidth(), mStrokeView.getHeight(), 1, path);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            uriForFile = FileProvider.getUriForFile(SDKManager.getInstance().getApplication(), SDKManager.getInstance().getPkgName() +".FileProvider", new File(path));
        } else {
        ...
        SDKManager.getInstance().getApplication().startActivity(Intent.createChooser(imageIntent, "分享"));
    }
    ...
    class CreateShareFileTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            if (platform.equals(QQ.NAME)) {
                if (AppInfoUtil.isQQClientAvailable(SDKManager.getInstance().getApplication())) {
                    nativeToShare(2, type);
                } else {
                    Toast.makeText(SDKManager.getInstance().getApplication(), R.string.share_qq_version_too_low
...
//                    shareParams.setImageUrl("");
        ...
        oks.setSite(SDKManager.getInstance().getApplication().getString(R.string.app_name));
        ...
        // 启动分享GUI
        oks.show(SDKManager.getInstance().getApplication());
