...
public class LoadingView extends View implements DialogInterface.OnClickListener {
    ...
    public LoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (isInEditMode()) {
            mColors = new int[1];
            mColors[0] = ContextCompat.getColor(context, R.color.migration_font_green);
        } else {
            mColors = getResources().getIntArray(colorArrayId);
        }

        colorId = ContextCompat.getColor(context, R.color.migration_font_green);
    ...
    public void draw(Canvas canvas) {
        ...
        if (mModeAppearing) {
            ...
            mPaint.setColor(gradient(colorId, colorId,
                    mCurrentSweepAngle / (360 - minSweepAngle * 2)));
    ...
    private Property<LoadingView, Float> mAngleProperty = new Property<LoadingView, Float>(Float.class, "angle") {
        @Override
        public Float get(LoadingView object) {
            return object.getCurrentGlobalAngle();
        }

        @Override
        public void set(LoadingView object, Float value) {
            object.setCurrentGlobalAngle(value);
        }
    };

    private Property<LoadingView, Float> mSweepProperty = new Property<LoadingView, Float>(Float.class, "arc") {
        @Override
        public Float get(LoadingView object) {
