...
public class TouchListener implements View.OnTouchListener {
    ...
    public TouchListener(CanvasFrame mCanvasFrame, int imgWidth, int imgHeight, int intrinsicWidth, int intrinsicHeight, GestureListener gestureListener) {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {// 单点监听和多点触碰监听
            ...
            case MotionEvent.ACTION_DOWN:
                mode = MODE_DRAG;
                // 记录ImageView当前的移动位置
                currentMatrix.set(canvasFrame.bDrawl.getMatrix());
...
//                    setDoubleTouchEvent(event);
                ...
                }else if (Math.abs(event.getY() - startPoint.y) < 300) {
                    L.info("slide mCurScale="+mCurScale);
                    if((event.getX() - startPoint.x)>200){
                        //向右
                        if(null!=gestureListener && mCurScale<=1.5){
                            gestureListener.rightDirect();
                        }
                    }else if((event.getX() - startPoint.x)<-200){
                        // 向左
                        if(null!=gestureListener && mCurScale<=1.5){
                            gestureListener.leftDirect();
                        }
                    }
                }
        ...
        canvasFrame.bDrawl.setMatrix(matrix);
        return true;
    }
