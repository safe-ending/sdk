 ...
 */
public class PageLabelItemRVAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    List<PageData> mList;//存放数据
    Context context;
    /**
     * 空数据时，显示空布局类型
     */
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (EMPTY_VIEW == viewType) {
            ...
            View view = LayoutInflater.from(context).inflate(R.layout.item_page_label, parent, false);
...
// 在这里对获取对象进行操作
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (VIEW_TYPE_PAGELABEL == itemViewType) {
            MyViewHolder myViewHolder = (MyViewHolder) holder;
            //设置textView显示内容为list里的对应项
            final PageData data = mList.get(position);
            if (data.name != null) {
                myViewHolder.textViewName.setText(data.name);
            }
...
/*        if (mViewType == EMPTY_VIEW) {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnLongClickListener {
        ...
        public MyViewHolder(View itemView, int viewType, OnRecyclerViewItemClickListener mListener, OnRecyclerItemLongListener longListener) {
            ...
            textViewName = itemView.findViewById(R.id.tv_label_name);
            ivThum = itemView.findViewById(R.id.iv_thumbtack);
            if(VIEW_TYPE_PAGELABEL ==viewType){
...
//        return false;
//    }
    public void setList(List<PageData> list, int viewType) {
