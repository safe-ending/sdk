...
//            }
    ...
    private void sendNQDot(NQDot dot) {
        ...
        if (defaultBg != SpUtils.getInt(SmartPenApp.getApp(), "DrawBg")) {
            SpUtils.putInt(SmartPenApp.getApp(), "DrawBg", defaultBg);
        }
        EventBusCarrier eventBusCarrier = new EventBusCarrier();
...
//                    AFPenClientCtrl.getInstance().getConnStatus() == PEN_CONN_STATUS.DISCONNECTED) {
            if (BluetoothClient.getBle().getBlueToothStatus()) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (!LocationUtils.isLocationEnabled(SmartPenApp.getApp())) {
                        return;
                    }
                }
                Log.e("重连", tryCount + "--");
